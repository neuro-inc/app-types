...
from apolo_app_types.helm.apps.common import get_preset, gen_extra_values


class LLMChartValueProcessor(BaseChartValueProcessor):
    ...
    def _configure_gpu_env(
        self, gpu_provider: str, gpu_count: int,
    ) -> dict[str, t.Any]:
    ...
    def _configure_parallel_args(
        ...
        parallel_server_args = []

        has_tensor_parallel = any("tensor-parallel-size" in arg for arg in parallel_server_args)
    ...
    def _configure_env(self, llm_model: LLMModel) -> dict[str, str]:
        return {
            "HUGGING_FACE_HUB_TOKEN": llm_model.hugging_face_model.hfToken
        }
    ...
    async def gen_extra_values(
        ...
        **kwargs: t.Any,
        ...
        parallel_args = self._configure_parallel_args(input_.llm.serverExtraArgs, gpu_count)
        server_extra_args = [
